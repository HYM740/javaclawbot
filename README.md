...
- **项目上下文**：开发者模式下自动加载项目的 CODE-AGENT.md/CLAUDE.md，支持 cwd 自动检测和手动指定
...
| `/stop` | 停止当前任务 |
| `/help` | 显示帮助信息 |
| `/project <path>` | 设置项目路径（开发者模式读取项目指令文件） |
| `/project clear` | 清除项目路径配置，恢复自动检测 |
...
项目路径通过以下方式确定（优先级从高到低）：

1. **配置持久化**：`agents.defaults.projectPath` 字段
2. **cwd 自动检测**：从当前工作目录向上查找包含 `.svn`、`CODE-AGENT.md` 或 `CLAUDE.md` 的目录
3. **手动指定**：使用 `/project <path>` 命令
